...
let logs =
  let doc =
    "set source-dependent logging level, eg: --logs *:info,foo:debug"
  in
...
let host =
  ...
  Arg.(
    required & opt (some string) None & info ["h"; "host"] ~doc ~docv:"HOST")

let fifo_name =
  let doc = "absolute path to fifo to write broadcast packet into" in
  Arg.(
    required & opt (some string) None & info ["f"; "file"] ~doc ~docv:"FIFO")
