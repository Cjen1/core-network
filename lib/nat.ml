...
let get_rule_handles t ip =
  if IpMap.mem ip t.rule_handles then
    Lwt.return @@ IpMap.find ip t.rule_handles
  else Lwt.return_nil
...
let translate t p_orig (buf, pkt) =
  ...
  | Ipv4 {ihl; raw= nat_buf; payload} ->
      ...
      | Udp {len; raw= udp_buf} ->
          let ph =
            Ipv4_packet.Marshal.pseudoheader ~src:src_ip ~dst:dst_ip
              ~proto:`UDP len
          in
...
let substitute t old_ip new_ip =
  let ntransl =
    IpPairMap.fold
      (fun kp vp n ->
        let nkp =
          if 0 = Ipaddr.V4.compare (fst kp) old_ip then (new_ip, snd kp)
          else kp
        in
        let nvp =
          if 0 = Ipaddr.V4.compare (snd vp) old_ip then (fst vp, new_ip)
          else vp
        in
        ...
        let nrules =
          List.map
            (fun (_src_ip, _dst_ip) ->
              ...
              else if 0 = Ipaddr.V4.compare _dst_ip old_ip then
                (_src_ip, new_ip)
